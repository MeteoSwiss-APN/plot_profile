...
1. Fork the `plot_profile` repo on GitHub.
...
3. Create a virtual environment and install the development dependencies::
    ...
    This will create a virtual environment named ``plot_profile`` (change with ``VENV_NAME=my_venv``) in ``./venv`` (change with ``VEND_DIR=path/to/venv``) and install the following dependencies:
...
.. list-table:: Structure
   ...
   * -  src/
     -  Source folder, with the main package in ``src/plot_profile``.
   * -  tests/
   ...
   * -  USAGE.txt
     -  Instruction on using plot_profile
   * -  VERSION
...
When you call it, the main function (``cli``) in ``src/plot_profile/cli.py`` is called.
...
If a user calls this script, the function ``main`` in the file ``src/plot_profile/cli.py`` is called.
